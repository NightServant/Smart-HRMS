...
/**
 * Seeds the active First Semester 2026 IPCR workflow records on top of the
 * finalized 2021–2025 history produced by HistoricalPerformanceSeeder.
 *
 * Distributes the 20 Admin Office employees across multiple workflow states
 * so the evaluator, HR, and PMT queues each have realistic pending items
 * to review without truncating historical data.
 */
class IpcrWorkflowSeeder extends Seeder
{
    ...
    private function resetCurrentPeriodWorkflow(): void
    {
        ...
        // Submissions that aren't fully finalized are considered "current"
        // workflow records and get wiped so the seeder can re-establish a
        // clean mix of pending stages. Historical (stage = finalized) rows
        // from prior periods stay untouched.
        DB::table('ipcr_appeals')
            ->whereIn('submission_id', function ($q): void {
                $q->select('id')
    ...
    {
        foreach ($employees as $index => $employee) {
            $hasActiveCurrentPeriod = IpcrSubmission::query()
                ->where('employee_id', $employee->employee_id)
                ->whereNotIn('stage', ['finalized'])
                ->exists();

            if ($hasActiveCurrentPeriod) {
                continue;
            }
    ...
    private function targetStateForIndex(int $index): array
    {
        return match ($index % 4) {
            ...
            2 => [
                ...
                'evaluator_remarks' => 'Targets approved and recorded by HR.',
