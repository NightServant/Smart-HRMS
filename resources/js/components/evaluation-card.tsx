...
export default function EvaluationCard({
    ...
    const derivedPassFail: 'passed' | 'failed' | null =
        averageScore === null ? null : averageScore >= 3.0 ? 'passed' : 'failed';
    const canSubmit = Boolean(employee && formPayload && ratedRows === totalRows && totalRows > 0 && remarks.trim() && derivedPassFail !== null && !processing);
    ...
    function handleSubmit(): void {
        ...
        router.post(
            saveEvaluation.url(),
            {
                ...
                evaluator_pass_fail: derivedPassFail,
        ...
        <Card className="glass-card mx-auto w-full min-w-0 max-w-7xl overflow-hidden border border-border bg-card shadow-sm">
            ...
            <CardContent className="space-y-5">
                ...
                <div className="glass-card rounded-[26px] border border-border bg-card p-5 shadow-sm">
                    <div className="mb-3 flex items-center gap-2">
                        <span className="size-2 rounded-full bg-[#4A7C3C] shadow-[0_0_0_4px_rgba(74,124,60,0.15)]" />
                        <label className="text-sm font-semibold text-foreground">Overall Assessment</label>
                    </div>
                    <p className="mb-4 text-xs text-muted-foreground">
                        Automatically determined from the computed Q/E/T average. The passing mark is{' '}
                        <strong>3.0 (Satisfactory)</strong> or above.
                    </p>
                    <div className="flex items-center gap-3">
                        {derivedPassFail === 'passed' && (
                            <span className="inline-flex items-center gap-2 rounded-full border border-emerald-500 bg-emerald-50 px-5 py-2 text-sm font-semibold text-emerald-700 dark:border-emerald-400 dark:bg-emerald-950/50 dark:text-emerald-300">
                                <span className="size-2.5 rounded-full bg-emerald-500" />
                                Passed
                            </span>
                        )}
                        {derivedPassFail === 'failed' && (
                            <span className="inline-flex items-center gap-2 rounded-full border border-rose-500 bg-rose-50 px-5 py-2 text-sm font-semibold text-rose-700 dark:border-rose-400 dark:bg-rose-950/50 dark:text-rose-300">
                                <span className="size-2.5 rounded-full bg-rose-500" />
                                Failed
                            </span>
                        )}
                        {derivedPassFail === null && (
                            <span className="inline-flex items-center gap-2 rounded-full border border-amber-400 bg-amber-50 px-5 py-2 text-sm font-semibold text-amber-700 dark:border-amber-400 dark:bg-amber-950/50 dark:text-amber-300">
                                <span className="size-2.5 rounded-full bg-amber-400" />
                                Pending — complete all ratings
                            </span>
                        )}
                    </div>
                </div>
