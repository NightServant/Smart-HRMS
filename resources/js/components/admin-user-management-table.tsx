...
export function AdminUserManagementTable({
    ...
    const [twoFactorFilter, setTwoFactorFilter] = useState(filters.twoFactor || 'all');
    ...
    const visit = (params: Partial<Filters> & { page?: number; perPage?: number }): void => {
        router.get(
            admin.userManagement().url,
            {
                ...
                role: (params.role ?? roleFilter) === 'all' ? '' : params.role ?? roleFilter,
                status: (params.status ?? statusFilter) === 'all' ? '' : params.status ?? statusFilter,
                twoFactor: (params.twoFactor ?? twoFactorFilter) === 'all' ? '' : params.twoFactor ?? twoFactorFilter,
    ...
    const renderSortIcon = (column: Filters['sort']) => {
        ...
        return filters.direction === 'asc' ? <ArrowUp className="size-4" /> : <ArrowDown className="size-4" />;
    };
    ...
    return (
        <>
            <div className="animate-slide-in-down top-0">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-muted-foreground">Manage all Smart HRMS accounts, roles, and account access from one place.</p>
                    </div>
            ...
            <div className="glass-card animate-zoom-in-soft mx-auto w-full rounded-md border border-border bg-card p-4 shadow-sm">
                <div className="grid gap-4 py-6 lg:grid-cols-[minmax(0,1fr)_repeat(3,11rem)]">
                ...
                <Table className="w-full">
                    <TableHeader>
                        <TableRow className="bg-[#2F5E2B] text-sm font-bold hover:bg-[#2F5E2B] dark:bg-[#1F3F1D] dark:hover:bg-[#1F3F1D] [&_th]:text-white">
                            ...
                            <TableHead>
                                <Button type="button" variant="ghost" size="sm" onClick={() => handleSortChange('role')} className="h-auto px-0 text-white hover:bg-transparent hover:text-white">
                                    Role
                                    {renderSortIcon('role')}
                                </Button>
                            </TableHead>
                            ...
                            <TableHead>
                                <Button type="button" variant="ghost" size="sm" onClick={() => handleSortChange('created_at')} className="h-auto px-0 text-white hover:bg-transparent hover:text-white">
                                    Created
                            ...
                            >
                                <TableCell>
                                    <div className="flex flex-col">
                                        <span>{user.name}</span>
                                        {user.position && <span className="text-xs font-normal text-muted-foreground">{user.position}</span>}
                                    </div>
                                </TableCell>
                                <TableCell>{user.email}</TableCell>
                                <TableCell className="capitalize">{user.role.replace('-', ' ')}</TableCell>
                                <TableCell>{user.employeeId ?? 'Not linked'}</TableCell>
            ...
            <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                <DialogContent className="sm:max-w-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>Create a new Smart HRMS account and assign its role and access state.</DialogDescription>
                    </DialogHeader>
                    <form className="grid gap-4" onSubmit={submitCreate}>
                        <div className="grid gap-4 md:grid-cols-2">
                            <div className="grid gap-2">
                                <Label htmlFor="create-name">Name</Label>
                                <Input id="create-name" value={createForm.name} onChange={(event) => setCreateForm((current) => ({ ...current, name: event.target.value }))} />
                                {errors.name && <p className="text-sm text-destructive">{errors.name}</p>}
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="create-email">Email</Label>
                                <Input id="create-email" type="email" value={createForm.email} onChange={(event) => setCreateForm((current) => ({ ...current, email: event.target.value }))} />
                            ...
                            <div className="grid gap-2">
                                ...
                                <Input id="create-employee-id" value={createForm.employee_id} onChange={(event) => setCreateForm((current) => ({ ...current, employee_id: event.target.value }))} placeholder="Optional employee link" />
                                {errors.employee_id && <p className="text-sm text-destructive">{errors.employee_id}</p>}
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="create-password">Password</Label>
                                <Input id="create-password" type="password" value={createForm.password} onChange={(event) => setCreateForm((current) => ({ ...current, password: event.target.value }))} />
                                {errors.password && <p className="text-sm text-destructive">{errors.password}</p>}
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="create-password-confirmation">Confirm Password</Label>
                                <Input id="create-password-confirmation" type="password" value={createForm.password_confirmation} onChange={(event) => setCreateForm((current) => ({ ...current, password_confirmation: event.target.value }))} />
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <Checkbox checked={createForm.is_active} onCheckedChange={(checked) => setCreateForm((current) => ({ ...current, is_active: checked === true }))} />
                    ...
                    <form className="grid gap-4" onSubmit={submitUpdate}>
                        <div className="grid gap-4 md:grid-cols-2">
                            <div className="grid gap-2">
                                ...
                                <Input id="edit-name" value={editForm.name} onChange={(event) => setEditForm((current) => ({ ...current, name: event.target.value }))} />
                                {errors.name && <p className="text-sm text-destructive">{errors.name}</p>}
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="edit-email">Email</Label>
                                <Input id="edit-email" type="email" value={editForm.email} onChange={(event) => setEditForm((current) => ({ ...current, email: event.target.value }))} />
                                {errors.email && <p className="text-sm text-destructive">{errors.email}</p>}
                            </div>
                            <div className="grid gap-2">
                        ...
                        <div className="flex items-center gap-3">
                            <Checkbox checked={editForm.is_active} onCheckedChange={(checked) => setEditForm((current) => ({ ...current, is_active: checked === true }))} />
