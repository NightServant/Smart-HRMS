...
test('remote enrollment falls back to the proxy when the open API returns 404', function () {
    config([
    ...
    Http::fake([
        ...
        // Proxy: no existing credential (duplicate guard) + trigger returns a session.
        'https://proxy.test/v1/employees/EMP200/fingerprints' => Http::response(['fingerprints' => []], 200),
    ...
    // The trigger is delegated to the proxy with the bearer key; Smart HRMS
    // never performs the Zlink login/cookie dance itself.
    Http::assertSent(fn ($request) => str_ends_with($request->url(), '/v1/fingerprints/enroll')
        && $request->method() === 'POST'
        && ($request->data()['employeeCode'] ?? null) === 'EMP200'
